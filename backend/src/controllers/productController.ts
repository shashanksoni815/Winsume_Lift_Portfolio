...
function buildPayload(data: z.infer<typeof productSchema>, heroImagePath?: string) {
  return {
    slug:               data.slug,
    name:               data.name,
...
export const createProduct = async (
  ...
  try {
    ...
    if (err instanceof z.ZodError) return next(createHttpError(400, "Invalid product data"));
    next(err);
  }
};
...
export const updateProduct = async (
  ...
  try {
    ...
    if (err instanceof z.ZodError) return next(createHttpError(400, "Invalid product update"));
    next(err);
  }
};
