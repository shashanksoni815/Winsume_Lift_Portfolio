...
export const listPublicBlogs = async (req: Request, res: Response, next: NextFunction): Promise<void> => {
  try {
    ...
    if (err instanceof z.ZodError) return next(createHttpError(400, "Invalid query parameters"));
    next(err);
  }
};

/** GET /api/blogs/public/categories */
export const listPublicCategories = async (_req: Request, res: Response, next: NextFunction): Promise<void> => {
  try {
...
export const getPublicBlog = async (req: Request, res: Response, next: NextFunction): Promise<void> => {
  try {
    ...
    if (!blog) throw createHttpError(404, "Blog post not found");
...
export const createBlog = async (req: Request, res: Response, next: NextFunction): Promise<void> => {
  try {
    ...
    if (err instanceof z.ZodError) return next(createHttpError(400, "Invalid blog data"));
    next(err);
  }
};
...
export const updateBlog = async (req: Request, res: Response, next: NextFunction): Promise<void> => {
  try {
    ...
    if (err instanceof z.ZodError) return next(createHttpError(400, "Invalid blog update data"));
    next(err);
  }
};

/** PATCH /api/blogs/:id/status */
export const updateBlogStatus = async (req: Request, res: Response, next: NextFunction): Promise<void> => {
  try {
    ...
    if (err instanceof z.ZodError) return next(createHttpError(400, "Invalid status value"));
    next(err);
  }
};
