...
export function EditPage() {
  ...
  const handleProductSubmit = async (event: React.FormEvent) => {
    ...
    try {
      ...
      if (editingProductId) {
        const updated: any = await adminFetch(`https://winsume-lift-backend01.onrender.com/api/products/${editingProductId}`, {
          method: 'PATCH',
          body: formData
        });
        updatedProducts = products.map(p => (p._id === editingProductId ? updated.product : p));
      } else {
        const created: any = await adminFetch('https://winsume-lift-backend01.onrender.com/api/products', {
          method: 'POST',
          body: formData
        });
  ...
  const handleDeleteProduct = async (productId?: string) => {
    ...
    try {
      setProductLoading(true);
      setProductError(null);
      await adminFetch(`https://winsume-lift-backend01.onrender.com/api/products/${productId}`, {
        method: 'DELETE'
      });
