...
export function AdminSidebar({ isCollapsed, onToggle }: AdminSidebarProps) {
  ...
  useEffect(() => {
    const loadCounts = async () => {
      try {
        ...
        if (projectsRes) {
          const data = await projectsRes.json().catch(() => null);
          setProjectCount(Array.isArray(data?.items) ? data.items.length : null);
        }
        if (inquiriesRes) {
          const data = await inquiriesRes.json().catch(() => null);
          setInquiryCount(Array.isArray(data?.items) ? data.items.length : null);
        }
        if (notificationsRes) {
          const data = await notificationsRes.json().catch(() => null);
          setNotificationCount(typeof data?.unreadCount === 'number' ? data.unreadCount : null);
        }
      } catch {
        // ignore
      }
  ...
  // ─── Navigate without touching collapsed state ────────────────────────────
  const handleNavClick = (path: string) => {
    navigate(path);
    // Do NOT call onToggle here — sidebar state is untouched
  };
    ...
    >
      ...
      <div className="p-6 border-b border-orange-500/20 flex-shrink-0">
        <div className="flex items-center justify-between">
          ...
          {/* ✅ Only this button controls collapse/expand */}
          <button
            onClick={() => onToggle(!isCollapsed)}
            className="p-2 rounded-lg bg-orange-500/10 hover:bg-orange-500/20 text-orange-500 transition-all flex-shrink-0"
          >
      ...
      <nav className="flex-1 overflow-y-auto py-6 px-3">
        <style>{`
          ...
          nav::-webkit-scrollbar-thumb { background: rgba(251,115,22,0.3); border-radius: 2px; }
          nav::-webkit-scrollbar-thumb:hover { background: rgba(251,115,22,0.5); }
        `}</style>

        {menuItems.map((item) => (
          <motion.button
            key={item.path}
            onClick={() => handleNavClick(item.path)}  // ✅ no collapse side-effect
            whileHover={{ x: isCollapsed ? 0 : 4 }}
            className={`w-full flex items-center gap-3 px-4 py-3 mb-2 rounded-lg transition-all ${
              isActive(item.path)
                ? 'bg-orange-500 text-white shadow-lg shadow-orange-500/20'
          ...
          <div className="relative">
            ...
            <AnimatePresence>
              {showUserDropdown && (
                ...
                >
                  <button
                    onClick={() => { navigate('/admin/profile'); setShowUserDropdown(false); }}
                    className="w-full flex items-center gap-3 px-4 py-3 text-white/80 hover:bg-orange-500/10 hover:text-white transition-all"
                  >
                    <User size={16} className="text-orange-500" />
                    <span className="text-sm">My Profile</span>
                  ...
                  <button
                    onClick={() => { navigate('/admin/account-settings'); setShowUserDropdown(false); }}
                    className="w-full flex items-center gap-3 px-4 py-3 text-white/80 hover:bg-orange-500/10 hover:text-white transition-all"
                  >
                    <Shield size={16} className="text-orange-500" />
                    <span className="text-sm">Account Settings</span>
